...
object GuiItems : ItemRegistry by Logistics.registry {
    ...
    private fun tpGuiItem(name: String): NovaItem =
        item(name) {
            localizedName("")
            hidden(true)
            models { selectModel { createGuiModel("item/$name", background = false, stretched = false) } }
        }
    ...
    private fun guiItem(name: String, localizedName: String = ""): NovaItem =
        item(name) {
            ...
            hidden(true)
            models { selectModel { createGuiModel("item/$name", background = true, stretched = false) } }
        }
