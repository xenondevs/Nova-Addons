...
import xyz.xenondevs.nova.addon.machines.recipe.group.hardcoded.StarCollectorRecipe.result
...
private val MAX_ENERGY = configReloadable { NovaConfig[AUTO_CRAFTER].getLong("max_energy") }
private val ENERGY_PER_TICK = configReloadable { NovaConfig[AUTO_CRAFTER].getLong("energy_per_tick") }
private val CRAFTING_SPEED = configReloadable { NovaConfig[AUTO_CRAFTER].getInt("speed") }
...
class AutoCrafter(blockState: NovaTileEntityState) : NetworkedTileEntity(blockState) {
    ...
    override val energyHolder = ConsumerEnergyHolder(
        ...
        null,
        upgradeHolder
    ) { createSideConfig(NetworkConnectionType.INSERT, BlockSide.BACK) }
    ...
    override fun handleInitialized(first: Boolean) {
        ...
        currentRecipe = Bukkit.getCraftingRecipe(recipeInv.items, world)
        val input = Bukkit.getCraftingRecipe(inputInv.items, world)
    ...
    private fun validateCraftingRecipe(event: ItemPostUpdateEvent) {
        ...
        val recipe = Bukkit.getCraftingRecipe(matrix, world)
        if (recipe != null) {
    ...
    private inner class AutoCrafterMenu : GlobalTileEntityMenu(GuiTextures.AUTO_CRAFTER) {
        ...
        private val inventoryWindow = Window.single().setGui(autoCrafterInventoryGui).setTitle(GuiTextures.AUTO_CRAFTER_INTERNAL_INVENTORY.getTitle("block.machines.auto_crafter.inv_window.title"))
    }
}